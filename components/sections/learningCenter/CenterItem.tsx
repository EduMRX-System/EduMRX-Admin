            ...
            <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                ...
                <td className="py-4 px-5">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <div className="font-semibold text-slate-900 dark:text-slate-100 leading-tight">{center.name}</div>
                            <div className="text-xs text-slate-400 dark:text-slate-500 mt-0.5 font-mono">/{center.slug}</div>
                        </div>
                    </div>
                </td>
                ...
                <td className="py-4 px-5">
                    <div className="space-y-1 text-xs text-slate-600 dark:text-slate-400">
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            <span>{center.students_count || 0} {t("centers.students")}</span>
                        </div>
                    </div>
                </td>
                ...
                {/* Status */}
                <td className="py-4 px-5">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold capitalize border ${center.status === "active"
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${center.status === "active" ? "bg-emerald-500" :
                            center.status === "pending" ? "bg-amber-500" : "bg-slate-400"
                            }`} />
            ...
            <div>
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold capitalize border ${center.status === "active"
                            ? "bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900"
                            : "bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border-amber-100 dark:border-amber-900"
                            }`}>
                            {t(`centers.status.${center.status}`)}
                ...
                {/* Markaz nomi */}
                <div className="mb-4">
                    <h4 className="font-bold text-slate-900 dark:text-slate-100 text-base line-clamp-1">{center.name}</h4>
                    <p className="text-xs text-slate-400 dark:text-slate-500 font-mono mt-0.5">/{center.slug}</p>
                </div>
